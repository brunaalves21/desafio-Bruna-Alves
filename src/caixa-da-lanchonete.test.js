...
describe('CaixaDaLanchonete', () => {
    ...
    const validaTeste = (formaDePagamento, resultadoEsperado, itens) => {
        ...
        expect(resultado.replace(/\xa0/g, ' ')).toEqual(resultadoEsperado);
    };
    ...
    test.each([
        ...
        test.each([
          ['dinheiro', 2.85, ['cafe,1']],
          ['credito', 3.09, ['cafe,1']],
          ['debito', 3.00, ['cafe,1']],
      ])('compra simples em %p deve resultar em R$ %p', (formaDePagamento, resultadoEsperado, itens) =>
          validaTeste(formaDePagamento, `R$ ${resultadoEsperado.toFixed(2)}`, itens));
       

          test.each([
            ['credito', 11.85, ['cafe,1', 'sanduiche,1', 'queijo,1']],
            ['debito', 11.50, ['cafe,1', 'sanduiche,1', 'queijo,1']],
        ])('compra de 3 itens em %p deve resultar em R$ %p', (formaDePagamento, resultadoEsperado, itens) =>
            validaTeste(formaDePagamento, `R$ ${resultadoEsperado.toFixed(2)}`, itens));
        
            test.each([
              ['dinheiro', 33.73, ['cafe,4', 'sanduiche,3', 'queijo,2']],
              ['credito', 36.56, ['cafe,4', 'sanduiche,3', 'queijo,2']],
              ['debito', 35.50, ['cafe,4', 'sanduiche,3', 'queijo,2']],
          ])('compra de múltiplas quantidades em %p deve resultar em R$ %p', (formaDePagamento, resultadoEsperado, itens) =>
              validaTeste(formaDePagamento, `R$ ${resultadoEsperado.toFixed(2)}`, itens));
